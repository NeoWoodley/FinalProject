...
int main() {
	...
	// 把索引复制到缓冲里，把这些函数调用放在绑定和解绑函数调用之间
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// 我们必须在每次要用索引渲染一个物体时绑定相应的EBO
		// VAO绑定时，正在绑定的索引缓冲对象会被保存为VAO的元素缓冲对象；绑定VAO的同时也会自动绑定EBO
		// 当目标是GL_ELEMENT_ARRAY_BUFFER时，VAO会储存glBindBuffer的函数调用；这意味着它也会储存解绑调用，要确保没有在解绑VAO前解绑索引数组缓冲，否则就没有这个EBO配置了
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
		glBindVertexArray(0);  // 解绑VAO，而不是EBO
