...
// 在回调函数中跟踪哪个键被按下/释放。在游戏循环中我们读取这些值，检查哪个按键被激活了，然后做出相应反应。创建一个布尔数组代表按下/释放的键
bool keys[1024];
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		// 图形和游戏应用通常有回跟踪一个deltaTime变量，
		// 它储存渲染上一帧所用的时间。
		// 我们把所有速度都去乘以deltaTime值。
		// 当我们的deltaTime变大时意味着上一帧渲染花了更多时间，
		// 所以这一帧使用这个更大的deltaTime的值乘以速度，会获得更高的速度，
		// 这样就与上一帧平衡了。
...
// 在key_callback函数中设置按下/释放键为true或false
// 所有的GLFW的按键枚举值都是整数，可以把它们当数组索引使用
void key_callback(GLFWwindow *window, int key, int scancode, int action, int mode) {
